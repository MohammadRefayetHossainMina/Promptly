...
This project follows the **Plan → Build → Test → Review → Fix → Continue**
cycle. Before writing or changing code:

1. Explain the plan in plain words first.
2. Build in small, reviewable steps — never the whole feature in one blast.
3. Only touch the files that step needs; preserve everything else.
4. After each step, say what changed, how to test it, and any known issues.
5. Prefer the simplest obvious solution; do not invent libraries or features.
6. If anything is ambiguous, ask before coding.

The full rules the founder chose to follow are saved in `master-prompt.md`
(at the project root). Treat that file as instructions too.
